...
const expectNotToChange = (value: any) => expect(omitTypename(value)).toEqual(value);
...
describe("omitTypename", () => {
  describe("When it's given a non-plain object it does nothing", () => {
    it("does not affect null", () => expectNotToChange(null));

    it("does not affect undefined", () => expectNotToChange(undefined));

    it("does not affect a boolean", () => expectNotToChange(true));

    it("does not affect a string", () => expectNotToChange("abc"));

    it("does not affect a number", () => expectNotToChange(123));
  ...
  it("does not affect objects without __typename", () => expectNotToChange({ bar: "baz" }));
